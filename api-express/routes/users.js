...
module.exports = (server) => {
  ...
  //delete user
  server.delete("/delUser/:id", async (req, res, next) => {
    try {
  ...
  //delete all users
  server.delete("/del-all-users", async (req, res, next) => {
    try {
      const found_user = await users.deleteMany();
      res.send(`${found_user.deletedCount}`);
      next();
    } catch (err) {
      return next(new errors.ResourceNotFoundError(`Delete Failed`));
    }
  });
};
